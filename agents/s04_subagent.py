...
SYSTEM = f"""You are a coding parent agent at {WORKDIR}.
...
3. If a command returns "(no output)", treat it as success unless there is other evidence of failure.
4. If output contains "already exists" or "已存在", treat the related step as DONE and do not retry.
5. Keep actions minimal, verify when needed, then return a short completion summary with key results.
6. Do not continue tool calls after the assigned task is complete.
"""
    ...
    while loop_count < max_loops:
        ...
        for block in response.content:
            ...
            try:
                ...
                # 处理任务分发工具调用
                if block.name == "task_dispatcher":
                    desc = block.input.get("description", "subtask")
                    sub_prompt = block.input.get("prompt")
                    print(f"> task ({desc}): {str(sub_prompt)[:80]}")
                    output = run_subagent(str(sub_prompt)) if sub_prompt else "Error: Missing required parameter: prompt"
                else:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            query = input(f"\033[36ms04 ({MODEL}) >> \033[0m")
